    ...
    run: async (client) => {
        ...
        try {
            if (CONFIG.devEnv.isDev) {
                ...
                console.log(`${chalk.green("[INFO]")} Set Commands for Dev Server\n${chalk.cyan("[LIST]")} (/) Commands List:`
                + `\n ${(await guild.commands.fetch()).map((c) => chalk.yellow(c.name)).join(", ")}\n`);
            } else {
                await client.application.commands.set(commands);
                console.log(`${chalk.green("[INFO]")} Set Commands for Production\n${chalk.cyan("[LIST]")} (/) Commands List:`
                + `\n ${(await client.application.commands.fetch()).map((c) => chalk.yellow(c.name)).join(", ")}\n`);
            }
        ...
        console.log(`${chalk.green("[INFO]")} Set Commands for Production\n${chalk.cyan("[LIST]")} (message) Commands List:`
        + `\n ${client.commands.map((c) => chalk.yellow(c.name)).join(", ")}\n`);
